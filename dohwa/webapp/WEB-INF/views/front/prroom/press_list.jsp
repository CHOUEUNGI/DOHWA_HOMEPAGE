...
<html lang="<%=response.getLocale().getLanguage()%>">
	...
	<body>
					...
					<div class="contents-wrap" style="position:relative">
						...
						<div class="contents no-btm-type clfix">
							...
							<div class="section-cont">
								<h3 class="hide-txt"><spring:message code="FRONT.COMMON.MENU.PRROOM.002.001.026" text="보도자료 검색결과" /></h3>
		...
		<script>
			...
			// 상세
			$('div.press-list ul').off('click').on("click", '[data-href]', function() {
				view($(this).data('href'));
			});
			
			// 상세
			function view(url) {
				sessionStorage.setItem("press", JSON.stringify({ c : _c, r : _r, url : url}));
		...
		function press() {
			...
			function removeStorage() {
				if(null !== sessionStorage.getItem("press")) {
			...
			function set(data, focus) {
				...
				if(typeof data.list !== 'undefined' && 0 < data.list.length) {
					...
					for(var i in data.list) {
						...
						s += '<li>';
						s += '	<div class= "press-content">';
						s += '		<div class="flex-box">';
						s += '			<div class="list-img-box">';
						s += '				<a class="list-link" href="javascript:void(0);" data-href="<c:url value="/prroom/press/" />'+ item.seq +'" title="">';
					
						if(item.mainFileName1!=null) {
						s += '					<span><img src="<c:url value="${WEB_URL}" />'+ item.mainFileName1 +'" alt="'+ item.title +'" />';	
							
						}else{
						s += '					<span><img src="/images/common/press_view_logo.png" alt="'+ item.title +'" /></span>';
						}
						s += '				</a>';
						s += '			</div>';
						s += '			<div class="list-content">';
						s += '				<p><span>'+ item.writeDt +'</span></p>';
						s +='				<a class="list-link" href="javascript:void(0);" data-href="<c:url value="/prroom/press/" />'+ item.seq +'" title="">';
						s += '					<dl>';
						s += '						<dt>';
						s += '							<strong>'+ item.title +'</strong>';
						s += '						</dt>';
						s += '						<dd class="ellipsis">'+ item.cont +'</dd>';
						s += '					</dl>';
						s += '				</a>';
						s += '			</div>';
						s += '		</div>';
						...
						if(i == 0) {
							firstDataHref = '<c:url value="/prroom/press/" />'+ item.seq;
						}
					}
				...
				$('div.press-list ul').append(s);

				if(typeof data.list !== 'undefined' && data.count <= $('div.press-list ul li').length) {
					$('._more_div').hide();
				}
				...
				if(!StringUtils.isEmpty(focus)){
					$('div.press-list a.list-link').each(function(){
			...
			function back() {
				var obj = JSON.parse(sessionStorage.getItem("press"));
				sessionStorage.removeItem("press");
